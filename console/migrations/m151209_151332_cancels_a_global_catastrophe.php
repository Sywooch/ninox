...
class m151209_151332_cancels_a_global_catastrophe extends Migration
{
    public function up()
    {
        ...
        $this->execute("UPDATE `partnersAddresses` `a`, `partners` `b` SET `a`.`name` = SPLIT_STR_TO_STR(`b`.`Company`, ' ', 1), `a`.`surname` = SPLIT_STR_TO_STR(`b`.`Company`, ' ', 2), `a`.`fathername` = SPLIT_STR_TO_STR(`b`.`Company`, ' ', 3), `a`.`partnerID` = `b`.`ID`, `a`.`default` = 1 WHERE `a`.`partnerID` = `b`.`Code`");
