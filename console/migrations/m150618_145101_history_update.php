...
UPDATE `history` SET `dostavka` = REPLACE(`dostavka`, ',', '') WHERE `dostavka` LIKE '%,%';
...
ALTER TABLE `history` DROP COLUMN `callState`;");
        ...
        $this->execute("ALTER TABLE `history`
            ...
            CHANGE `dostavka` `deliveryType` INT(4) NOT NULL DEFAULT 0,
            ...
            CHANGE `perezvon` `callback` INT(4) NOT NULL DEFAULT 0,
            CHANGE `zamena` `canChangeItems` INT(2) NOT NULL DEFAULT 0,
            CHANGE `fakt_summ` `actualAmount` DOUBLE(24, 2) NOT NULL DEFAULT 0,
            ...
            CHANGE `sendsms` `smsState` INT(4) NOT NULL DEFAULT 0,
            CHANGE `trash` `deleted` INT(4) NOT NULL DEFAULT 0,
